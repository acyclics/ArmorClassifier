...
class ArmorDetector():
    ...
    def __init__(self):
        ...
        self.network = Network(nclasses, img_size=img_size, anchors=anchors)
    ...
    def actual_eval_hsv_bw(self):
        ...
        for _ in range(100):
            # Get groundtruth
            image, y_true_13, y_true_26, y_true_52, og_image = self.dataset.get_groundTruth(self.dataset.sample_gt_idx(),
                                                      [self.dataset.img_wh[0], self.dataset.img_wh[1]],
                                                      self.dataset.anchors)
            # Edit image
            """
            #image = cv2.resize(image, self.dataset.og_img_wh)
            img = image * 255.0
            ...
            mask4 = cv2.inRange(hsv_img, (0, 0, 25), (172, 111, 255))
            ...
            image = cropped_img / 255.0
            """
            # Eval
            boxes, probs, classes = self(np.asarray([image]), False)
            ...
            for l, b in zip(classes[0], boxes[0]):
                ...
                bx_max, by_max = b[3], b[2]
                b1 = [by_min, bx_min, by_max, bx_max]
                image = cv2.rectangle(image, (b1[1], b1[0]), (b1[3], b1[2]), (0, 255, 0), 2)
                l = self.labels_to_data[np.argmax(l)]
                cv2.putText(image, str(l), (int(b1[1]), int(b1[0]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,0), 3)
            print("Displaying Image")
            cv2.imshow("Image", image)
